    ...
    private void addStructureDataFromGenerator(ServerWorld world, BlockPos playerPos, int maxChunkRange)
    {
        ...
        for (int cz = minCZ; cz <= maxCZ; ++cz)
        {
            for (int cx = minCX; cx <= maxCX; ++cx)
            {
                ...
                for (Map.Entry<StructureFeature<?>, StructureStart<?>> entry : chunk.getStructureStarts().entrySet())
                {
                    StructureFeature<?> structure = entry.getKey();
                    StructureStart<?> start = entry.getValue();
                    Identifier id = world.getRegistryManager().get(Registry.STRUCTURE_FEATURE_KEY).getId(structure);
